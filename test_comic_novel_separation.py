"""
Test script to verify comics and novels are properly separated
"""
if __name__ == '__main__':
    ...
    # Test 1: Comics homepage should only show comics
    print("1. Testing Comics Homepage (/comics-home)")
try:
    response = requests.get(f'{base_url}/comics-home')
    if response.status_code == 200:
        soup = BeautifulSoup(response.text, 'html.parser')
        # Find all comic titles on the page
        comic_elements = soup.find_all(class_='card-title')
        print(f"   ✓ Page loaded successfully")
        print(f"   Found {len(comic_elements)} comics displayed")
...
# Test 2: Novels homepage should only show novels
print("\n2. Testing Novels Homepage (/novels-home)")
try:
    response = requests.get(f'{base_url}/novels-home')
    if response.status_code == 200:
        soup = BeautifulSoup(response.text, 'html.parser')
        print(f"   ✓ Page loaded successfully")
...
# Test 3: API ranking endpoint with type filter
print("\n3. Testing API Ranking - Comics Only")
try:
    response = requests.get(f'{base_url}/api/comic-ranking?type=comics&period=all')
    if response.status_code == 200:
        data = response.json()
        if data.get('success'):
...
# Test 4: API ranking endpoint with novels filter
print("\n4. Testing API Ranking - Novels Only")
try:
    response = requests.get(f'{base_url}/api/comic-ranking?type=novels&period=all')
    if response.status_code == 200:
        data = response.json()
        if data.get('success'):
